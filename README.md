...
本项目把企业年报、招股说明书、行业报告和历史案例转换为可追溯 EvidenceUnit，建立企业事实与行业背景，并面向审批方向生成同行比较、审批点评价和综合风险判断。
...
- [Ontology 设计规范](ontology/科技型企业通用Ontology设计规范_精简版_V0.8.md)
...
```powershell
python scripts/run_historical_profile.py --help
python scripts/run_current_profile.py --help
python scripts/run_react_current_profile.py --help
python scripts/review_profile_run.py --help
python scripts/build_comparison_card.py --help
python scripts/search_similar_profiles.py --help
python scripts/compare_similar_profiles.py --help
python -m pytest -q
```
...
src/profiles/             企业画像、比较和报告
src/industry/             行业背景画像
src/case_analysis/        历史案例分析
src/ontology/             Ontology 加载与校验
ontology/                 Ontology 规范与清单
prompts/                  当前模型协议
scripts/                  命令行入口
